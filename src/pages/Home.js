        ...
        <>
            ...
            <section className={styles.home_container}>
                <form className={styles.form_card} onSubmit={onSubmit}>
                    ...
                    />
                    <button className={styles.buttons} onClick={clickRepos}>
                        Repositorios
                    </button>
                    <button className={styles.buttons} onClick={clickStarred}>
                        Starred
                    </button>
                </form>
            </section>
                ...
                starred.map((starr) => (
                    <div className={styles.homeCard} key={starr.id}>
                        ...
                        <div className={styles.homeCardDown}>
                            {starr.description ? (
                                <p>{starr.description}</p>
                            ) : (
                                <p>Sem descrição</p>
                            )}
                        </div>
                        <div className={styles.link}>
                            <a href={starr.html_url}>Link:{starr.html_url}</a>
                        </div>
            ...
            {!hideRepos &&
                repos.map((rep) => (
                    <div className={styles.homeCard} key={rep.id}>
                        <h4>Postado por: {rep.owner.login}</h4>
                        <div className={styles.homeCardDown}>
                            {rep.description ? (
                                <p>{rep.description}</p>
                            ) : (
                        ...
                        <div className={styles.link}>
                            <a href={rep.html_url}>Link:{rep.html_url}</a>
                        </div>
                        <div className={styles.data}>
